...
func main() {
	...
	body := fmt.Sprintf("{\"Params\":[\"%s\",\"zjlx\",\"\"]}", code_id)
	jsonData := []byte(body)
	...
	stmt, err := tx.Prepare("INSERT INTO " + dbTable + " (code, date, jlr, jlrzb, zljlr, zljlrzb, cddjlr, cddjlrzb, ddjlr, ddjlrzb) VALUES (?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE name = name;")
	if err != nil {
		log.Fatal(err)
	}
	defer stmt.Close()
